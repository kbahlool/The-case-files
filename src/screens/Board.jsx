...
export default function Board() {
  const { cs, prog, go, addLink } = useGame();
  const [pick, setPick] = useState(null); const [msg, setMsg] = useState(null); const [reveal, setReveal] = useState(null);
  const nodes = useMemo(() => {
    const out = [];
    cs.suspects.forEach((s, i) => out.push({ id: 'S:' + s.id, label: s.name.split(' ')[0], sub: 'Suspect', x: 30 + i * 155, y: 20, kind: 'sus' }));
    const locs = cs.locations.filter(l => nodeAvailable(cs, prog, 'L:' + l.id));
    locs.forEach((l, i) => out.push({ id: 'L:' + l.id, label: l.name, sub: 'Location', x: 30 + i * 155, y: 100, kind: 'loc' }));
    cs.evidence.filter(e => prog.evidence.includes(e.id)).forEach((e, i) => out.push({ id: e.id, label: e.title, sub: e.category, x: 20 + (i % COLS) * (CW + 6), y: 190 + Math.floor(i / COLS) * RH, kind: 'ev', ico: CAT_ICON[e.category] }));
    return out;
  ...
  const tap = (n) => {
    ...
    if (l && nodeAvailable(cs, prog, l.a) && nodeAvailable(cs, prog, l.b)) {
      ...
      setReveal(l);
    } else { audio.sfx('failure'); setMsg('No clear connection between those two. Try another pairing.'); setTimeout(() => setMsg(null), 2200); }
    setPick(null);
  };
  const total = cs.boardLinks.length;
  return (
    <Shell title={`Board · ${made.length}/${total}`} tab="board" onBack={() => go('hub', {}, true)}>
      <p className="board-hint">{pick ? 'Now tap what it connects to.' : 'Tap one item, then another, to test a connection.'}{msg && <b className="fb"> {msg}</b>}</p>
      ...
      <Sheet open={!!reveal} onClose={() => setReveal(null)} title={reveal?.title}>
        {reveal && <><p>{reveal.insight}</p><button className="btn primary" onClick={() => setReveal(null)}>Continue</button></>}
      </Sheet>
    </Shell>
  );
}
