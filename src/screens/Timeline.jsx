...
export default function Timeline() {
  const { cs, prog, go, setTimeline, t, lang } = useGame();
  ...
  const check = () => {
    if (vis.length < minEvents) { setFb(`${t('You only have')} ${vis.length} ${t('events.')} ${t('Keep investigating, you need at least')} ${minEvents}.`); audio.sfx('failure'); return; }
    ...
    else { setFb(bad ? t('One timestamp cannot be trusted. Discard the one that was faked, and keep the others.') : t('Some events are out of order. Compare each with the evidence behind it.')); audio.sfx('failure'); }
  };
  const solved = prog.timeline.solved;
  return (
    <Shell title={t('Timeline')} tab="timeline" onBack={() => go('hub', {}, true)}>
      <div className="scroll pad">
        <p className="lead-text">{t('Order the night. Use the arrows to move an event, and Discard any time that was faked.')}</p>
        {vis.length < minEvents && <p className="muted small">{vis.length} {t('of at least')} {minEvents} {t('events known.')}</p>}
        <ol className="tl">
          {order.map((id, i) => { const e = ev(id); const off = flags[id]; return (
            <li key={id} className={`tl-row ${off ? 'off' : ''} ${solved ? 'ok' : ''}`}>
              ...
              {!solved && <div className="tl-ctl">
                <button onClick={() => move(i, -1)} aria-label={lang === 'ar' ? 'تحريك للأعلى' : 'Move up'} disabled={i === 0}>▲</button>
                <button onClick={() => move(i, 1)} aria-label={lang === 'ar' ? 'تحريك للأسفل' : 'Move down'} disabled={i === order.length - 1}>▼</button>
                <button className={`disc ${off ? 'on' : ''}`} onClick={() => flip(id)}>{off ? t('Discarded') : t('Discard')}</button>
              </div>}
            </li>); })}
        </ol>
        {solved ? <div className="win"><small>{t('TIMELINE ESTABLISHED')}</small><p>{lang === 'ar' ? 'وقعت الجريمة نحو 10:40 مساءً، قبل أكثر من ساعة مما تقوله الساعة. توقيت 11:47 مُلفّق.' : 'The killing happened around 10:40 PM, more than an hour before the clock says. The 11:47 is fabricated.'}</p>
          <button className="btn primary big" onClick={() => go('board', {}, true)}>{t('Continue to the Board')}</button></div>
          : <><button className="btn primary big" onClick={check}>{t('Check timeline')}</button>{fb && fb !== 'ok' && <p className="fb">{fb}</p>}</>}
      </div>
    </Shell>
  );
}
