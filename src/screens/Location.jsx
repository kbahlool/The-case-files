...
export default function Location({ id }) {
  const { cs, prog, go, back, visit, examine, grant, settings } = useGame();
    ...
    <Shell title={loc.name} tab="hub" onBack={() => go('hub', {}, true)} right={<span className="counter">{got}/{total}</span>}>
      ...
      <Sheet open={!!sel} onClose={() => setSel(null)} title={sel?.label}>
        {sel && (unlocked(sel) ? <>
          ...
          {sel.evidence?.length > 0 && <p className="found">{sel.evidence.map(e => cs.evidence.find(x => x.id === e)?.title).join(' · ')} added to evidence.</p>}
          {sel.puzzle && !solved(sel) && <button className="btn primary" onClick={() => { setSel(null); go('puzzle', { id: sel.puzzle, loc: id }); }}>Examine closely</button>}
        </> : <p>{sel.lockedText || 'Nothing of interest yet.'}</p>)}
