...
export default function Interrogation({ id }) {
  const { cs, prog, ask, back, settings, t, lang } = useGame();
  const s = cs.suspects.find(x => x.id === id); const script = cs.interrogations[id];
  const you = lang === 'ar' ? 'أنت' : 'YOU';
  ...
  const pick = (q) => {
    ...
    setLines(l => [...l, { who: you, text: q.text, you: true }, ...q.response.map(line => ({ who: s.name.split(' ')[0].toUpperCase(), text: line, mood: q.mood })),
      ...(q.reveals?.length ? [{ sys: true, text: `${lang === 'ar' ? 'دليل جديد' : 'New evidence'}: ${q.reveals.map(r => cs.evidence.find(e => e.id === r)?.title).join(lang === 'ar' ? '، ' : ', ')}` }] : [])]);
  };
    ...
    <Shell title={s.name} tab="suspects" onBack={back} noNav>
      <div className="interro" data-mood={mood}>
        <div className="int-stage"><Backdrop set="interrogation" caseId={cs.id} dur={50000} reduce={settings.reduceMotion} /><Fx list={['grain']} />
          ...
          <div className="susp-bar" aria-label={`Suspicion ${prog.suspicion[id] || 0}%`}><i style={{ width: (prog.suspicion[id] || 0) + '%' }} /><small>{t('SUSPICION')}</small></div>
        </div>
        ...
        <div className="int-q">
          {avail.length === 0 ? <p className="muted small center">{t('Nothing more to ask right now. Find new evidence, then return.')}</p>
            : avail.map(({ q }) => <button key={q.id} className={`q ${q.requires?.length ? 'evid' : ''}`} onClick={() => pick(q)}>{q.requires?.length ? '▤ ' : ''}{q.text}</button>)}
          <button className="btn" onClick={back}>{t('End interview')}</button>
        </div>
      </div>
    </Shell>
  );
