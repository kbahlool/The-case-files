...
function CountWheel({ lever = 10, label }) {
  ...
  return (
    <figure className="exhibit wheel">
      ...
      <figcaption>{t('Lever rests at')} <b>{lever}</b> · {label}</figcaption>
    </figure>
  );
}
...
function Code({ p, hintOn, onSolve, onFail, attempts }) {
  ...
  return (
    <div className="code">
      ...
      {hint ? <p className="hint-box">{hint}</p> : <p className="muted small">{hintOn ? t('You know where to look. Try the code.') : t('Wrong code? Explore the workshop for a clue.')}</p>}
    </div>
  );
}
function Choice({ p, onSolve, onFail }) {
  ...
  const submit = () => { const o = p.options.find(x => x.id === sel); if (o.correct) onSolve(); else { setFb(o.feedback || t('That does not hold up.')); onFail(); } };
  return (
    <div className="choice">
      ...
      <button className="btn primary" disabled={!sel} onClick={submit}>{t('Submit answer')}</button>
    </div>
  );
}
function Order({ p, onSolve, onFail }) {
  ...
  const check = () => { const right = list.filter((id, i) => id === p.order[i]).length; if (right === p.order.length) onSolve(); else { setFb(lang === 'ar' ? `${right} ${t('of')} ${p.order.length} ${t('clips are in the right place.')}` : `${right} of ${p.order.length} clips are in the right place.`); onFail(); } };
  return (
    <div className="order">
      {list.map((id, i) => <div className="ord-row" key={id}><span className="ord-n">{i + 1}</span><span className="ord-t">{p.items.find(x => x.id === id).text}</span>
        <span className="ord-b"><button onClick={() => move(i, -1)} aria-label={t('Move earlier')} disabled={i === 0}>▲</button><button onClick={() => move(i, 1)} aria-label={t('Move later')} disabled={i === list.length - 1}>▼</button></span></div>)}
      {fb && <p className="fb bad" role="alert">{fb}</p>}
      <button className="btn primary" onClick={check}>{t('Check the order')}</button>
    </div>
  );
}
function Match({ p, onSolve, onFail }) {
  ...
  const check = () => { const right = p.left.filter(l => p.right.find(r => r.id === pairs[l.id])?.match === l.id).length; if (right === p.left.length) onSolve(); else { setFb(lang === 'ar' ? `${right} ${t('of')} ${p.left.length} ${t('pairs are right.')}` : `${right} of ${p.left.length} pairs are right.`); onFail(); } };
  return (
    <div className="match">
      <p className="muted small">{t('Tap a suspect, then tap what their alibi really covers.')}</p>
      ...
      {fb && <p className="fb bad" role="alert">{fb}</p>}
      <button className="btn primary" disabled={Object.keys(pairs).length < p.left.length} onClick={check}>{t('Check alibis')}</button>
    </div>
  );
}
